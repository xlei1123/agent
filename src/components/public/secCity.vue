  ...
  <transition name="fade">
    <div class="cityBox">
      <div class="hotCity clearfix">
        ...
        <ul>
          <li v-for="(city,index) in hotCity" @click="secHotCity(index)">{{city}}</li>
        </ul>
      </div>
      <div class="moreCity">
        <ul>
          <li v-for="(citys,word) in cityList" @click=secW(word)>
            <a :id="'anchor-'+word" class="wordH">{{word}}</a>
            <ul class="wordCity">
              <li v-for="(city,index) in citys" @click="secCity(index)">{{city}}</li>
            </ul>
          </li>
        </ul>
      </div>
      <div class="quickly">
        <ul>
          <li v-for="letter in letterList">
            <a href="javascript:void(0)" @click="goAnchor('#anchor-'+letter)">{{letter}}</a>
          </li>
        </ul>
      </div>
    </div>
...
<script>
  ...
  export default{
    data(){
      return {
        hotCity:['北京','上海','广州','深圳','杭州','南京','天津','武汉','重庆'],
      ...
      "保定","宝鸡","保山","包头","巴中","北海","蚌埠","本溪","毕节","滨州","百色","亳州"],
          ...
          'J':["济南","佳木斯","吉安","江门","焦作","嘉兴","嘉峪关","揭阳","吉林","金昌","晋城","景德镇","荆门","荆州","金华","济宁","晋中","锦州","九江",  
      "酒泉"],
          'K':["昆明","开封"],
          'L':["兰州","拉萨","来宾","莱芜","廊坊","乐山","凉山","连云港",  
      "聊城","辽阳","辽源","丽江","临沧","临汾","临夏","临沂","林芝","丽水","六安","六盘水",  
      "柳州","陇南","龙岩","娄底","漯河","洛阳","泸州","吕梁"],
          'M':["马鞍山","茂名","眉山","梅州",  
      "绵阳","牡丹江"],
          'N':["南京","南昌","南宁","宁波","南充","南平","南通","南阳","那曲","内江",  
      "宁德","怒江"],
    ...
    methods:{
      ...
      goAnchor(selector) {
        ...
        this.remind(selector.slice(selector.indexOf('-')+1))
        document.body.scrollTop = anchor.offsetTop
      },
      secHotCity(index){
        var _this=this;
        localStorage.setItem('addbankcity',_this.hotCity[index]);
        router.go(-1);
      },
      secCity(index){
      ...
      secW(word){
        var _this=this;
        localStorage.setItem('addbankcity',_this.cityList[word][_this.cityIndex]);
        router.go(-1);
      },
      close(){
        this.bankL=false;
